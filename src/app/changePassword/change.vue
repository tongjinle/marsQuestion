  ...
  <div class="changePassword">	  
  	...
  	<div class="password" v-if="!login">
	  	<form action="" method="get">
	  		原 密 码: 　<input type="password" name="" v-model="oldPassword"><br><!--v-on:blur="ready"-->
  ...
  methods:{
   ...
   //      };
    ...
    saveInfo: function() {  //保存信息
    	if(this.newPassword!=""&&this.conPassword!=""&&this.oldPassword!=""&&this.passwordFlag==false&&this.conPasswordFlag==false){   //如果输入的内容都对,将新密码传给服务器
     ...
     var urlDict={};
      ...
      var data={
          ...
          token:this.sendTokenMsg
          };

    		this.$http.post(urlDict.login,data).then((response)=> {          
          console.log(response.data.flag);
          if(response.data.flag){   //如果收到true,修改密码成功
            this.login=true;  //密码修改成功显示
          }else{
            this.wrong=true;
          }
